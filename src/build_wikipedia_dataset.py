...
def fetch_article_extracts(entries: list[dict[str, str]], pause: float = 0.1) -> list[dict[str, str]]:
    ...
    # Reduce batch size to 20 to comply with Wikipedia's 'extracts' limit
    batches = batched(entries, size=20) 
    ...
    for i, batch in enumerate(batches):
        ...
        payload = fetch_json({
            "action": "query",
            "prop": "extracts|info",
            "exchars": "1200",         # Requesting 1200-1500 is safer for API stability
            "exlimit": "20",           # CRITICAL: This allows all 20 titles to return text
            "explaintext": "1",
            "inprop": "url",
            "redirects": "1",
            "titles": "|".join(titles),
...
def make_rows(pages: list[dict[str, str]], include_control: bool = True) -> tuple[list[dict[str, str]], dict[str, Any]]:
    ...
    for page in pages:
        # Skip if the Wikipedia extract came back empty
